...
public class ColdWallet {
    ...
    public static void useCase(Investor investor){
        ...
        while (repeat) {
            ...
            boolean checkYesNo = checkresponse(response, "yes", "no", "\nWrong option");
            while (!checkYesNo) {
                response = input("input Yes or No!");
                checkYesNo = checkresponse(response, "yes", "no", "\nWrong option");
            }
            ...
            while (!checkreponse) {
                 walletName = input("\nFrom which wallet would yu like to transfer?");
                checkreponse = checkWalletName(walletName, investor);
            }
            sendCold(investor,walletName);

            String repeatTrans = input("\nWould you like to send crypto to a cold wallet again?(Yes/No)");
    ...
    public static void sendCold(Investor investor, String walletName){
        String cryptoName = input("\nWhat Crypto would you like to send to the cold wallet?");
        ...
        input("\nEnter the address of the cold wallet");
        print("\nVerifying destination wallet address.......");
        wait(1500);
        print("\n"+cryptoAmount+" "+cryptoName+" "+"succesfully transfered to cold wallet!");
        wait(100);
        investor.tradeCryto(walletName, cryptoName, -Double.parseDouble(cryptoAmount), "send");
